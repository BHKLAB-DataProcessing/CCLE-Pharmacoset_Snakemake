...
rna_tpm <- MultiAssayExperiment::experiments(assay_mae)[["rnaseq.gene_tpm"]]
rna_counts <- tryCatch(
  MultiAssayExperiment::experiments(assay_mae)[["rnaseq.gene_counts"]],
...
cell_meta <- CoreGx::cellInfo(pset) |>
  ...
  rename(
    site_primary = CCLE.site_Primary,
    histology = CCLE.histology,
    depmap_id = CCLE.depMapID,
    cell_line = cellosaurus.cellLineName
  ) |>
...
```{r lineage-composition, fig.width=8, fig.height=5.5}
lineage_counts <- sample_meta |>
  left_join(cell_meta, by = "sampleid") |>
  mutate(tissue = coalesce(site_primary, histology, "unspecified")) |>
  count(tissue, sort = TRUE) |>
  mutate(
    tissue = factor(tissue, levels = tissue),
    label = sprintf("%s (n=%s)", tissue, scales::comma(n))
  )
...
ggplot(lineage_counts, aes(x = "", y = n, fill = tissue)) +
  ...
  coord_polar(theta = "y") +
  scale_fill_manual(
    values = tissue_palette(lineage_counts$tissue),
    labels = lineage_counts$label
  ) +
...
feature_missing_df <- tibble(
  feature = names(feature_miss),
  missing = feature_miss
)
...
*   **Shape:** Gene expression (TPM) distributions are typically right-skewed, with many genes having low expression and a few highly expressed genes. The `log2(TPM + 0.001)` transformation often makes the distribution more symmetric.
*   **Dynamic Range:** A wide distribution indicates a good dynamic range of expression levels captured by the assay.
*   **Why plots can look flat:** If density is computed on linear TPM and only the x-axis is log-transformed, the density can appear flattened. Here we compute density directly on `log2(TPM + 0.001)` values for a more interpretable shape.
...
log_vals <- log2(vals + 1e-3)

ggplot(tibble(log_tpm = log_vals), aes(x = log_tpm)) +
...
var_df <- tibble(
  ...
  mutate(log_variance = log1p(variance))

ggplot(var_df, aes(x = log_variance)) +
  geom_histogram(bins = 40, fill = palette_classic[5], color = "white") +
  labs(
    title = "Variance of top genes",
    subtitle = sprintf("Top %s HVGs", scales::comma(nrow(var_df))),
    x = "log1p(Variance of TPM)",
    y = "Gene count"
  )
```
...
sample_means_log2 <- log2(sample_means + 1e-3)
ggplot(
  tibble(sampleid = names(sample_means), mean_tpm_log2 = sample_means_log2),
  aes(x = mean_tpm_log2)
) +
  geom_histogram(bins = 40, fill = col_mean_tpm, color = "white") +
  labs(
    title = "Sample mean TPM",
    x = "Mean log2(TPM + 0.001)",
    y = "Samples"
  )
```
...
if (ncol(mat) > 2) {
  ...
  if (!is.null(pca)) {
    ...
    pca_df <- as_tibble(pca$x[, 1:2], rownames = "sampleid") |>
      ...
      left_join(cell_meta, by = "sampleid") |>
      mutate(
        tissue = coalesce(site_primary, histology, "unspecified")
      )
...
```{r correlation-heatmap, fig.width=7.5, fig.height=6}
...
pheatmap(
  cor_subset,
  color = colorRampPalette(c("white", "#B2182B"))(201),
...
```{r replicate-correlations}
replicate_meta <- sample_meta |>
  left_join(cell_meta, by = "sampleid") |>
  filter(!is.na(cell_line))

replicate_groups <- replicate_meta |>
  count(cell_line, name = "n") |>
  filter(n > 1)

if (nrow(replicate_groups)) {
  rep_corr <- replicate_meta |>
    filter(cell_line %in% replicate_groups$cell_line) |>
    group_by(cell_line) |>
    summarise(
      pairs = list(combn(sampleid, 2, simplify = FALSE)),
      .groups = "drop"
    ) |>
    unnest(pairs) |>
    transmute(
      cell_line = cell_line,
      sample_a = map_chr(pairs, 1),
      sample_b = map_chr(pairs, 2),
      correlation = map_dbl(
        pairs,
        ~ cor_mat[.x[1], .x[2]]
      )
  ...
  if (nrow(rep_corr)) {
    DT::datatable(
      rep_corr,
      caption = "Replicate pair correlations (cell lines with >=2 samples).",
      options = list(dom = "t", pageLength = 10)
    )
  } else {
    DT::datatable(
      tibble(Message = "Replicate pairs not found after filtering."),
      options = list(dom = "t")
    )
  }
} else {
  DT::datatable(
    tibble(Message = "No cell lines with replicate samples detected."),
    options = list(dom = "t")
  )
}
```
...
if (nrow(hk_vals)) {
  hk_vals <- hk_vals |>
    mutate(log2_tpm = log2(tpm + 1e-3))

  ggplot(hk_vals, aes(x = gene, y = log2_tpm, fill = gene)) +
...
if (length(available_gc_col) > 0 || length(available_length_col) > 0) {
  tpm_mean_log2 <- tibble(
    ...
    mean_tpm_log2 = log2(rowMeans(tpm_mat, na.rm = TRUE) + 1e-3)
  ) |>
