...
mut_se <- MultiAssayExperiment::experiments(assay_mae)[["mutation.gene_binary"]]
cnv_se <- MultiAssayExperiment::experiments(assay_mae)[["cnv.gene_log2"]]
...
nonzero_fraction <- if (is.numeric(mut_mat) || is.logical(mut_mat)) {
  mean(mut_mat > 0, na.rm = TRUE)
} else {
  mean(
    !is.na(mut_mat) &
      mut_mat != "" &
...
mut_present <- if (is.numeric(mut_mat) || is.logical(mut_mat)) {
  mut_mat > 0
} else {
  !is.na(mut_mat) &
    mut_mat != "" &
...
if (length(available_allele_cols) >= 2) {
  ...
  if (file.exists(maf_path)) {
    maf_df <- readr::read_tsv(maf_path, comment = "#", show_col_types = FALSE)
    maf_cols <- intersect(allele_cols, names(maf_df))
    if (length(maf_cols) >= 2) {
      ref_col <- maf_cols[1]
      alt_col <- maf_cols[2]
      maf_df <- maf_df |>
        select(!!sym(ref_col), !!sym(alt_col)) |>
        filter(
          nchar(!!sym(ref_col)) == 1,
          nchar(!!sym(alt_col)) == 1,
          !!sym(ref_col) %in% c("A", "C", "G", "T"),
          !!sym(alt_col) %in% c("A", "C", "G", "T")
        ) |>
        mutate(
          is_transition = case_when(
            (!!sym(ref_col) %in% purine &
      ...
      num_transitions <- sum(maf_df$is_transition, na.rm = TRUE)
      num_transversions <- sum(maf_df$is_transversion, na.rm = TRUE)
      titv_ratio <- if (num_transversions > 0) {
        num_transitions / num_transversions
      } else {
        NA_real_
      ...
      DT::datatable(
        ...
        colnames = c("Metric", "Value"),
        caption = "Transition/Transversion (Ti/Tv) Ratio Summary (MAF fallback).",
        options = list(dom = 't')
      )
    } else {
      DT::datatable(
        tibble(
          Message = "MAF file found, but allele columns were not present for Ti/Tv calculation."
        ),
        options = list(dom = 't')
      )
    }
...
*   **Mutation Status Axis:** The x-axis is a logical flag derived from the mutation calls (`mut_binary == 1`). **TRUE** means the sample is mutated for that gene; **FALSE** means no mutation call (wild-type / not mutated) for that gene.
...
plot_concordance <- function(g) {
  ...
  ggplot(trimmed, aes(x = mutated, y = cnv_trim)) +
    ...
    labs(
      ...
      x = "Mutated (TRUE = mutated, FALSE = wild-type)",
      y = "CNV value"
    )
}
