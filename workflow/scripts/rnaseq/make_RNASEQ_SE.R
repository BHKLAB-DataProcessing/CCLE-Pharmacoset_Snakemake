...
stopifnot(
  length(setdiff(transcripts_tpm[, transcript_id], gencode_GR$transcript_id)) ==
    0
)
...
rowRanges <- gencode_GR[
  gencode_GR$type == "gene" & gencode_GR$gene_id %in% rownames(countsMatrix),
]
...
rowRanges <- gencode_GR[
  gencode_GR$type == "gene" & gencode_GR$gene_id %in% rownames(rpkmMatrix),
]
